...
| Website chat | Built in; **no outside service** | A chat button on every public page, in English or Tagalog. It answers questions about each procedure: what it is, how long it takes, whether it hurts, recovery, and how long it lasts (`app/chat_procedures.py`), plus common patient questions (`app/chat_faq.py`), searches the patient guides, gives branch details and hours, and quotes starting prices from **Administration → Price list**. Prices reach patients on the live site only after a super admin replaces the sample prices and turns on "Show prices to patients" (the demo shows them labelled as samples). It never gives medical advice, steers every answer toward booking (pre-filled with the branch and treatment), and "Have our team call me" saves a lead (source *Website chat*). Conversations aren't stored. | Nothing. Edit procedures in `app/chat_procedures.py` and questions in `app/chat_faq.py`. |
