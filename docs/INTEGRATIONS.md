...
| Website chat | Built in; **no outside service** | A chat button on every public page, in English or Tagalog. It answers common patient questions (`app/chat_faq.py`), searches the patient guides, gives branch details and hours, and quotes starting prices from **Administration → Price list**. Prices reach patients on the live site only after a super admin replaces the sample prices and turns on "Show prices to patients" (the demo shows them labelled as samples). It never gives medical advice, steers every answer toward booking (pre-filled with the branch and treatment), and "Have our team call me" saves a lead (source *Website chat*). Conversations aren't stored. | Nothing. Add or edit questions in `app/chat_faq.py`. |
